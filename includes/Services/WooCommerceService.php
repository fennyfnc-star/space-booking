...
{
    /**
     * Track created product IDs for cleanup after payment.
     * @var array
     */
    ...
    {
        // Reset tracking for new booking
        ...
        $create_product = function ($name, $price, $item_breakdown = [], $extra_id = 0, $start_time = '', $end_time = '', $duration_hours = 0, $thumbnail_url = '') use ($booking_id, $booking_data) {
            ...
            // Track product ID for cleanup after payment
            $this->created_product_ids[] = $product->get_id();

            // Mark creation time for orphan cleanup
            $product->add_meta_data('_sb_created', time(), true);
